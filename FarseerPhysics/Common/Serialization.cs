...
namespace FarseerPhysics.Common
{
    public static class WorldSerializer
    {
        ...
        public static async void Serialize(World world, string filename)
        {
            ...
            {
                using (var outStream = fs.AsStream()/*.GetOutputStreamAt(0).AsStream()*/)
                {
                    new WorldXmlSerializer().Serialize(world, outStream);
                }
            }
        }
#else
        ...
        public static async void Deserialize(World world, string filename)
        {
            ...
            using (var fs = await file.OpenAsync(FileAccessMode.Read))
            {
                using (var outStream = fs.AsStream()/*.GetOutputStreamAt(0).AsStream()*/)
                {
                    new WorldXmlDeserializer().Deserialize(world, outStream);
                }
            }
           
        }
        public static async System.Threading.Tasks.Task<World> Deserialize(string filename)
        {
            var file = await ApplicationData.Current.LocalFolder.GetFileAsync(filename);
            using (var fs = await file.OpenAsync(FileAccessMode.Read))
            {
                using (var outStream = fs.AsStream()/*.GetOutputStreamAt(0).AsStream()*/)
                {
                    return new WorldXmlDeserializer().Deserialize(outStream);
                }
            }
        }
#else
    ...
    public class XMLFragmentParser
    {
        ...
        {
            System.Threading.Tasks.Task task = System.Threading.Tasks.Task.Factory.StartNew(async () => {
                var file = await ApplicationData.Current.LocalFolder.GetFileAsync(fileName);
                using (var fs = await file.OpenAsync(FileAccessMode.Read))
                {
                    using (var outStream = fs.AsStream()/*.GetOutputStreamAt(0).AsStream()*/)
